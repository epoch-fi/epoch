...
const CustomTooltip: React.FC<CustomTooltipProps> = ({ active, payload }) => {
  if (active && payload && payload.length) {
    return (
      <div className="custom-tooltip flex flex-col items-center justify-center gap-1 border border-green-500 bg-black px-3 py-1 text-center">
        <span className="text-[10px] font-bold text-green-500">Apple Inc.</span>
        <p className="text-white">{payload[0].payload.value}</p>
        <p className="text-[10px] text-white">{payload[0].payload.time}</p>
      </div>
    );
  }
};
      ...
      >
        <defs>
          <linearGradient id="colorPv" x1="0" y1="0" x2="0" y2="1">
            <stop offset="5%" stopColor="#59d759" stopOpacity={0.8} />
            <stop offset="95%" stopColor="#59d759" stopOpacity={0} />
          </linearGradient>
        </defs>
        {/* <CartesianGrid strokeDasharray="3 3" /> */}
        <XAxis dataKey="time" minTickGap={31} />
